...
FC7PayloadUnpacker::FC7PayloadUnpacker() 
    : PayloadUnpacker()
    , FC7HeaderPtrCol_(std::make_shared<data_products::common::DataProductPtrCollection>())
    , FC7HeaderParser_(std::make_unique<unpackers::wfd5::FC7HeaderParser>())
{
...
int FC7PayloadUnpacker::Unpack(const uint64_t* words, unsigned int& wordNum) {
    ...
    */
    ...
    auto header_words = unpackers::common::GetXWords(words,wordNum,3,"bigendian");
