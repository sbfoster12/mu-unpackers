...
WFD5CRBankUnpacker::WFD5CRBankUnpacker() :
    BankUnpacker(),
    className_("WFD5CRBankUnpacker")
{
    LoggerHolder::getInstance().InfoLogger << "We are constructing the CR bank unpacker." << std::endl;

    //Initialize the unpackers
    payloadUnpackers_[WFD5_BOARD_TYPE] = std::make_unique<unpackers::WFD5PayloadUnpacker>();
    payloadUnpackers_[FC7_BOARD_TYPE] = std::make_unique<unpackers::FC7PayloadUnpacker>();
